    ...
    ILogger<QuestionService> logger) : IQuestionService
{
    ...
    public async Task<CreateManyQuestionsResultDto> CreateManyAsync(CreateManyQuestionsDto dto)
    {
        var result = new CreateManyQuestionsResultDto();

        var test = await unitOfWork.Tests.SelectAsync(t => t.Id == dto.TestId);
        if (test is null) throw new MilliyMockException(404, "Test not found");

        var insertedGroups = new List<QuestionGroup>();
        var insertedQuestions = new List<Question>();

        // --- Phase 1: Question Groups ---
        if (dto.QuestionGroups is not null)
        {
            foreach (var groupDto in dto.QuestionGroups)
            {
                try
                {
                    ...
                    if (groupDto.TitleUz is not null)
                    {
                        string? imagePathUz = groupDto.ImageUz is not null
                            ? await fileService.UploadImage(groupDto.ImageUz)
                            : null;

                        questionGroup.Translations.Add(new Translation
                        {
                            Language = Language.Uzbek,
                            Text = groupDto.TitleUz,
                            ImagePath = imagePathUz,
                            QuestionGroup = questionGroup
                        });
                    }

                    if (groupDto.TitleRu is not null)
                    {
                        string? imagePathRu = groupDto.ImageRu is not null
                            ? await fileService.UploadImage(groupDto.ImageRu)
                            : null;

                        questionGroup.Translations.Add(new Translation
                        {
                            Language = Language.Russian,
                            Text = groupDto.TitleRu,
                            ImagePath = imagePathRu,
                            QuestionGroup = questionGroup
                        });
                    }

                    if (groupDto.Options is not null && groupDto.Options.Count > 0)
                    {
                        questionGroup.Options = mapper.Map<List<Option>>(groupDto.Options);
                        foreach (var option in questionGroup.Options)
                            option.CreatedBy = HttpContextHelper.UserId;
                    }

                    var groupExplanation = new QuestionExplanation { QuestionGroup = questionGroup };

                    if (groupDto.Explanation is not null)
                    {
                        groupExplanation.Translations = new List<Translation>
                        {
                            new()
                            {
                                Text = groupDto.Explanation.TextUz, Language = Language.Uzbek,
                                QuestionExplanation = groupExplanation
                            },
                            new()
                            {
                                Text = groupDto.Explanation.TextRu, Language = Language.Russian,
                                QuestionExplanation = groupExplanation
                            }
                        };
                    }

                    await unitOfWork.QuestionExplanations.InsertAsync(groupExplanation);

                    if (groupDto.Questions is not null && groupDto.Questions.Count > 0)
                    {
                        ...
                        foreach (var questionDto in groupDto.Questions)
                        {
                            ...
                            {
                                string? imagePathUz = questionDto.ImageUz is not null
                                    ? await fileService.UploadImage(questionDto.ImageUz)
                                    : null;

                                question.Translations = new List<Translation>
                                {
                                    new()
                                    {
                                        Language = Language.Uzbek, Text = questionDto.TextUz, ImagePath = imagePathUz,
                                        Question = question
                                    },
                                    new()
                                    {
                                        Language = Language.Russian, Text = questionDto.TextUz, Question = question
                                    }
                                };
                            }

                            var questionExplanation = new QuestionExplanation { Question = question };

                            if (questionDto.Explanation is not null)
                            {
                                questionExplanation.Translations = new List<Translation>
                                {
                                    new()
                                    {
                                        Text = questionDto.Explanation.TextUz, Language = Language.Uzbek,
                                        QuestionExplanation = questionExplanation
                                    },
                                    new()
                                    {
                                        Text = questionDto.Explanation.TextRu, Language = Language.Russian,
                                        QuestionExplanation = questionExplanation
                                    }
                                };
                            }
                    ...
                    insertedGroups.Add(questionGroup);
                }
                catch (Exception ex)
                {
                    logger.LogError(ex, "Failed to prepare question group {title}", groupDto.TitleUz);
                    result.FailCount++;
                }
            }
        }
                ...
                try
                {
                    ...
                    if (questionDto.TextUz is not null)
                    {
                        ...
                        question.Translations = new List<Translation>
                        {
                            ...
                            new() { Language = Language.Russian, Text = questionDto.TextUz, Question = question }
                        };
                    }

                    if (questionDto.Options is not null && questionDto.Options.Count > 0)
                    {
                        question.Options = mapper.Map<List<Option>>(questionDto.Options);
                        foreach (var option in question.Options)
                            option.CreatedBy = HttpContextHelper.UserId;
                    }
                    ...
                    if (questionDto.Explanation is not null)
                    {
                        explanation.Translations = new List<Translation>
                        {
                            new()
                            {
                                Text = questionDto.Explanation.TextUz, Language = Language.Uzbek,
                    ...
                    await unitOfWork.QuestionExplanations.InsertAsync(explanation);
                    await unitOfWork.Questions.InsertAsync(question);
                    insertedQuestions.Add(question);
                }
                catch (Exception ex)
                {
                    logger.LogError(ex, "Failed to prepare standalone question {text}", questionDto.TextUz);
                    result.FailCount++;
                }
            }
        }

        if (insertedGroups.Count > 0 || insertedQuestions.Count > 0)
            await unitOfWork.SaveChangesAsync();

        result.CreatedQuestionGroups = mapper.Map<List<QuestionGroupResultDto>>(insertedGroups);
        result.CreatedQuestions = mapper.Map<List<QuestionResultDto>>(insertedQuestions);
        ...
        try
        {
            ...
            if (dto.Explanation is not null)
            {
                if (question.QuestionExplanation is null)
                {
                    ...
                    explanation.Translations = new List<Translation>
                        { explanationTranslationUz, explanationTranslationRu };
                }
                else
                {
                    await UpdateQuestionExplanationTranslationAsync(question.QuestionExplanation, Language.Uzbek,
                        dto.Explanation.TextUz);
                    await UpdateQuestionExplanationTranslationAsync(question.QuestionExplanation, Language.Russian,
                        dto.Explanation.TextUz);
                }
            }
    ...
    private async Task UpdateTranslationAsync(Question question, Language language, string? newText,
        IFormFile? newImage)
    {
